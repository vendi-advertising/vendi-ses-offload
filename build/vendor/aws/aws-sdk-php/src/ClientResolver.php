...
use Vendi\SesOffload\Vendor\Aws\Api\ApiProvider;
use Vendi\SesOffload\Vendor\Aws\Api\Service;
use Vendi\SesOffload\Vendor\Aws\Api\Validator;
use Vendi\SesOffload\Vendor\Aws\Auth\AuthResolver;
use Vendi\SesOffload\Vendor\Aws\Auth\AuthSchemeResolver;
use Vendi\SesOffload\Vendor\Aws\Auth\AuthSchemeResolverInterface;
use Vendi\SesOffload\Vendor\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use Vendi\SesOffload\Vendor\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use Vendi\SesOffload\Vendor\Aws\ClientSideMonitoring\Configuration;
use Vendi\SesOffload\Vendor\Aws\Configuration\ConfigurationResolver;
use Vendi\SesOffload\Vendor\Aws\Credentials\CredentialProvider;
use Vendi\SesOffload\Vendor\Aws\Credentials\Credentials;
use Vendi\SesOffload\Vendor\Aws\Credentials\CredentialsInterface;
use Vendi\SesOffload\Vendor\Aws\DefaultsMode\ConfigurationInterface as ConfigModeInterface;
use Vendi\SesOffload\Vendor\Aws\DefaultsMode\ConfigurationProvider as ConfigModeProvider;
use Vendi\SesOffload\Vendor\Aws\Endpoint\EndpointProvider;
use Vendi\SesOffload\Vendor\Aws\Endpoint\PartitionEndpointProvider;
use Vendi\SesOffload\Vendor\Aws\Endpoint\UseDualstackEndpoint\Configuration as UseDualStackEndpointConfiguration;
use Vendi\SesOffload\Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationInterface as UseDualStackEndpointConfigurationInterface;
use Vendi\SesOffload\Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider as UseDualStackConfigProvider;
use Vendi\SesOffload\Vendor\Aws\Endpoint\UseFipsEndpoint\Configuration as UseFipsEndpointConfiguration;
use Vendi\SesOffload\Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationInterface as UseFipsEndpointConfigurationInterface;
use Vendi\SesOffload\Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider as UseFipsConfigProvider;
use Vendi\SesOffload\Vendor\Aws\EndpointDiscovery\ConfigurationInterface;
use Vendi\SesOffload\Vendor\Aws\EndpointDiscovery\ConfigurationProvider;
use Vendi\SesOffload\Vendor\Aws\EndpointV2\EndpointDefinitionProvider;
use Vendi\SesOffload\Vendor\Aws\Exception\AwsException;
use Vendi\SesOffload\Vendor\Aws\Exception\InvalidRegionException;
use Vendi\SesOffload\Vendor\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use Vendi\SesOffload\Vendor\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
use Vendi\SesOffload\Vendor\Aws\Signature\SignatureProvider;
use Vendi\SesOffload\Vendor\Aws\Token\Token;
use Vendi\SesOffload\Vendor\Aws\Token\TokenInterface;
use Vendi\SesOffload\Vendor\Aws\Token\BedrockTokenProvider;
use Vendi\SesOffload\Vendor\Aws\Token\TokenProvider;
...
class ClientResolver
{
    ...
    {
        if ($value === \false) {
            $value = new Configuration(\false, \Vendi\SesOffload\Vendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \Vendi\SesOffload\Vendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \Vendi\SesOffload\Vendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
    ...
    public static function _apply_endpoint_provider($value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            if ($value instanceof \Vendi\SesOffload\Vendor\Aws\EndpointV2\EndpointProviderV2) {
            ...
            $args['region'] = \Vendi\SesOffload\Vendor\Aws\strip_fips_pseudo_regions($args['region']);
    ...
    public static function _default_endpoint_provider(array $args)
    {
        ...
        if (self::isValidService($serviceName) && self::isValidApiVersion($serviceName, $apiVersion)) {
            ...
            return new \Vendi\SesOffload\Vendor\Aws\EndpointV2\EndpointProviderV2($ruleset, EndpointDefinitionProvider::getPartitions());
        }
        $options = self::getEndpointProviderOptions($args);
    ...
    public static function _apply_suppress_php_deprecation_warning($value, &$args)
    {
        if ($value) {
            ...
            $args['suppress_php_deprecation_warning'] = \Vendi\SesOffload\Vendor\Aws\boolean_value(getenv("AWS_SUPPRESS_PHP_DEPRECATION_WARNING"));
        } elseif (!empty($_SERVER["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"])) {
            $args['suppress_php_deprecation_warning'] = \Vendi\SesOffload\Vendor\Aws\boolean_value($_SERVER["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"]);
        } elseif (!empty($_ENV["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"])) {
            $args['suppress_php_deprecation_warning'] = \Vendi\SesOffload\Vendor\Aws\boolean_value($_ENV["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"]);
        }
    }
    public static function _default_endpoint(array &$args)
    {
        ...
        $serviceIdentifier = \Vendi\SesOffload\Vendor\Aws\manifest($args['service'])['serviceIdentifier'];
...
EOT;
    ...
    private static function isValidService($service)
    {
        ...
        $services = \Vendi\SesOffload\Vendor\Aws\manifest();
