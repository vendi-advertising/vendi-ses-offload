...
use Vendi\SesOffload\Vendor\Aws\Api\Parser\PayloadParserTrait;
...
use Vendi\SesOffload\Vendor\Aws\S3\Exception\PermanentRedirectException;
use Vendi\SesOffload\Vendor\Aws\S3\Exception\S3Exception;
use Vendi\SesOffload\Vendor\GuzzleHttp\Promise\PromiseInterface;
use Vendi\SesOffload\Vendor\GuzzleHttp\Promise\RejectedPromise;
...
trait S3ClientTrait
{
    ...
    public function deleteMatchingObjectsAsync($bucket, $prefix = '', $regex = '', array $options = [])
    {
        ...
        if ($regex) {
            $iter = \Vendi\SesOffload\Vendor\Aws\filter($iter, function ($c) use ($regex) {
