...
use Vendi\SesOffload\Vendor\Aws\HashingStream;
use Vendi\SesOffload\Vendor\Aws\PhpHash;
use Vendi\SesOffload\Vendor\Aws\ResultInterface;
use Vendi\SesOffload\Vendor\Aws\S3\ApplyChecksumMiddleware;
...
use Vendi\SesOffload\Vendor\GuzzleHttp\Promise\Promise;
use Vendi\SesOffload\Vendor\GuzzleHttp\Promise\PromiseInterface;
use Vendi\SesOffload\Vendor\GuzzleHttp\Psr7\LazyOpenStream;
use Vendi\SesOffload\Vendor\GuzzleHttp\Psr7\LimitStream;
use Vendi\SesOffload\Vendor\GuzzleHttp\Psr7\Utils;
...
final class MultipartUploader extends AbstractMultipartUploader
{
     ...
     */
    public function __construct(S3ClientInterface $s3Client, array $requestArgs, string|StreamInterface $source, array $config = [], ?TransferListenerNotifier $listenerNotifier = null, ?ResumableUpload $resumableUpload = null)
    {
        ...
        if ($this->isResuming) {
            ...
            if (!empty($snapshotData)) {
                $currentSnapshot = TransferProgressSnapshot::fromArray($snapshotData);
            }
            $calculatedObjectSize = $resumableUpload->getObjectSize();
            $isFullObjectChecksum = $resumableUpload->isFullObjectChecksum();
        }
        parent::__construct($s3Client, $requestArgs, $config, $uploadId, $partsCompleted, $currentSnapshot, $listenerNotifier);
        $this->source = $source;
    ...
    protected function completeMultipartOperation(): PromiseInterface
    {
        ...
        $completeMultipartUploadArgs['MultipartUpload'] = ['Parts' => array_values($this->partsCompleted)];
    ...
    private function evaluateCustomChecksum(): void
    {
        ...
        $checksumName = ApplyChecksumMiddleware::filterChecksum($this->requestArgs);
    ...
    private function createUploadPartPromises(array $uploadPartCommandArgs, int $partSize, int $partsCount): \Generator
    {
        ...
        while (!$this->body->eof()) {
            ...
            yield $this->s3Client->executeAsync($command)->then(function (ResultInterface $result) use ($command, $partBody) {
                ...
                $partData = $this->collectPart($result, $command);
                // Part Upload Completed Event
                $this->partCompleted($command['ContentLength'], $command->toArray(), $partData);
     ...
     * @return void
     */
    protected function partCompleted(int $partSize, array $requestArgs, array $partData): void
    {
    ...
    {
        return ($this->config['resume_enabled'] ?? \false) && is_string($this->source);
    }
    /**
    ...
    private function persistResumeState(array $partData): void
    {
        if ($this->resumableUpload === null) {
            ...
            $sourceSize = $this->body->getSize() ?? $this->calculatedObjectSize;
            $this->resumableUpload = new ResumableUpload($resumeFilePath, $this->requestArgs, $this->config, $this->currentSnapshot->toArray(), $this->uploadId, $this->partsCompleted, $this->source, $sourceSize, $this->calculatePartSize(), $this->isFullObjectChecksum);
        }
        // Update the completed parts and current snapshot
        $this->resumableUpload->markPartCompleted($partData['PartNumber'], $partData);
        $this->resumableUpload->updateCurrentSnapshot($this->currentSnapshot->toArray());
        // Save to file
