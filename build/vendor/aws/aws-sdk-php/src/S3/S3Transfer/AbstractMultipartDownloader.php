...
use Vendi\SesOffload\Vendor\Aws\S3\S3Transfer\Models\DownloadResult;
...
use Vendi\SesOffload\Vendor\Aws\S3\S3Transfer\Utils\ResumableDownloadHandlerInterface;
use Vendi\SesOffload\Vendor\Aws\S3\S3Transfer\Utils\AbstractDownloadHandler;
use Vendi\SesOffload\Vendor\Aws\S3\S3Transfer\Utils\StreamDownloadHandler;
use Vendi\SesOffload\Vendor\GuzzleHttp\Promise\Coroutine;
use Vendi\SesOffload\Vendor\GuzzleHttp\Promise\Create;
...
abstract class AbstractMultipartDownloader implements PromisorInterface
{
    ...
    public function __construct(protected readonly S3ClientInterface $s3Client, array $downloadRequestArgs, array $config = [], ?AbstractDownloadHandler $downloadHandler = null, array $partsCompleted = [], int $objectPartsCount = 0, int $objectSizeInBytes = 0, ?string $eTag = null, ?TransferProgressSnapshot $currentSnapshot = null, ?TransferListenerNotifier $listenerNotifier = null, ?ResumableDownload $resumableDownload = null)
    {
        ...
        if ($this->isResuming) {
            ...
            if (!empty($snapshotData)) {
                $this->currentSnapshot = TransferProgressSnapshot::fromArray($snapshotData);
            }
        } else {
    ...
    public function promise(): PromiseInterface
    {
        return Coroutine::of(function () {
            ...
            $concurrency = $this->downloadHandler->isConcurrencySupported() ? $this->config['concurrency'] : 1;
            yield Each::ofLimitAll($partsDownloadPromises, $concurrency)->then(function () {
                // Transfer completed
                $this->downloadComplete();
                return Create::promiseFor(new DownloadResult($this->downloadHandler->getHandlerResult(), $this->initialRequestResult));
    ...
    protected function initialRequest(): PromiseInterface
    {
        ...
        return $this->s3Client->executeAsync($command)->then(function (ResultInterface $result) use ($command) {
            ...
            $initialRequestResult['ContentRange'] = "0-" . ($this->objectSizeInBytes - 1) . "/" . $this->objectSizeInBytes;
            ...
            $this->partDownloadCompleted(1, $result->toArray(), $command->toArray());
    ...
    private function partDownloadRequests(): \Generator
    {
        while ($this->currentPartNo < $this->objectPartsCount) {
            ...
            $command = $this->getNextGetObjectCommand();
            yield $this->s3Client->executeAsync($command)->then(function (ResultInterface $result) use ($command, $partNumber) {
                $requestArgs = $command->toArray();
                // Remove metadata
                unset($result['@metadata']);
                $this->partDownloadCompleted($partNumber, $result->toArray(), $requestArgs);
            });
        }
    ...
    private function getNextGetObjectCommand(): CommandInterface
    {
        ...
        return $this->s3Client->getCommand(self::GET_OBJECT_COMMAND, $nextCommandArgs);
    }
    /**
    ...
    private function downloadInitiated(array $commandArgs): void
    {
        ...
        // Prepare context
        $context = [AbstractTransferListener::REQUEST_ARGS_KEY => $commandArgs, AbstractTransferListener::PROGRESS_SNAPSHOT_KEY => $this->currentSnapshot];
    ...
    private function downloadFailed(\Throwable $reason): void
    {
        ...
        // Prepare context
        $context = [AbstractTransferListener::REQUEST_ARGS_KEY => $this->downloadRequestArgs, AbstractTransferListener::PROGRESS_SNAPSHOT_KEY => $this->currentSnapshot, AbstractTransferListener::REASON_KEY => $reason];
        // Notify download handler
     ...
     * @return void
     */
    private function partDownloadCompleted(int $partNumber, array $result, array $requestArgs): void
    {
        $partTransferredBytes = $result['ContentLength'] ?? 0;
        // Snapshot and context for listeners
        $newSnapshot = new TransferProgressSnapshot($this->currentSnapshot->getIdentifier(), $this->currentSnapshot->getTransferredBytes() + $partTransferredBytes, $this->objectSizeInBytes, $this->initialRequestResult);
        $this->currentSnapshot = $newSnapshot;
        // Notify download handler and evaluate if part was written
        $downloadHandlerSnapshot = $this->currentSnapshot->withResponse($result);
        $wasPartWritten = $this->downloadHandler->bytesTransferred([AbstractTransferListener::REQUEST_ARGS_KEY => $requestArgs, AbstractTransferListener::PROGRESS_SNAPSHOT_KEY => $downloadHandlerSnapshot]);
        // If part was written to destination then we mark it as completed
        if ($wasPartWritten) {
            $this->partsCompleted[$partNumber] = \true;
            // Persist resume state just if resume is enabled
            if ($this->config['resume_enabled'] ?? \false) {
                // Update the resume state holder
                $this->resumableDownload?->updateCurrentSnapshot($this->currentSnapshot->toArray());
        ...
        // Notify listeners
        $this->listenerNotifier?->bytesTransferred([AbstractTransferListener::REQUEST_ARGS_KEY => $this->downloadRequestArgs, AbstractTransferListener::PROGRESS_SNAPSHOT_KEY => $this->currentSnapshot]);
    }
    /**
    ...
    private function downloadComplete(): void
    {
        ...
        $this->currentSnapshot = $newSnapshot;
        // Prepare context
        $context = [AbstractTransferListener::REQUEST_ARGS_KEY => $this->downloadRequestArgs, AbstractTransferListener::PROGRESS_SNAPSHOT_KEY => $this->currentSnapshot];
        // Notify download handler
     ...
     * @return int
     */
    public static function computeObjectSizeFromContentRange(string $contentRange): int
    {
