...
use Vendi\SesOffload\Vendor\Aws\S3\S3Transfer\AbstractMultipartDownloader;
use Vendi\SesOffload\Vendor\Aws\S3\S3Transfer\Exception\FileDownloadException;
use Vendi\SesOffload\Vendor\Aws\S3\S3Transfer\Progress\AbstractTransferListener;
final class FileDownloadHandler extends AbstractDownloadHandler implements ResumableDownloadHandlerInterface
{
     ...
     */
    public function __construct(string $destination, bool $failsWhenDestinationExists, bool $resumeEnabled = \false, ?string $temporaryFilePath = null, ?int $fixedPartSize = null)
    {
    ...
    private function openExistingFile(): void
    {
        ...
        if ($handle === \false) {
            throw new FileDownloadException("Failed to open existing temporary file '{$this->temporaryFilePath}' for resuming.");
        }
        $this->handle = $handle;
    }
    /**
    ...
    public function bytesTransferred(array $context): bool
    {
        ...
        if ($this->handle === null) {
            throw new FileDownloadException("Failed to initialize destination for downloading.");
        }
    ...
    {
        $objectSize = AbstractMultipartDownloader::computeObjectSizeFromContentRange($response['ContentRange'] ?? "");
    ...
    private function writePartToDestinationHandle(array $response): bool
    {
        ...
        if ($contentRange === null) {
            throw new FileDownloadException("Unable to get content range from response.");
        }
        $partNo = (int) ceil(AbstractMultipartDownloader::getRangeTo($contentRange) / $this->fixedPartSize);
        ...
        try {
            ...
            $checksumParameter = ApplyChecksumMiddleware::filterChecksum($response);
            $hashContext = null;
            if ($checksumParameter !== null) {
                $checksumAlgorithm = strtolower(str_replace("Checksum", "", $checksumParameter));
                $checksumAlgorithm = $checksumAlgorithm === 'crc32' ? 'crc32b' : $checksumAlgorithm;
            ...
            if ($hashContext !== null) {
                $calculatedChecksum = base64_encode(hash_final($hashContext, \true));
                if ($calculatedChecksum !== $response[$checksumParameter]) {
                    throw new FileDownloadException("Checksum mismatch when writing part to destination file.");
                }
            }
            fflush($this->handle);
            return \true;
    ...
    private function ensureDirectoryExists(): void
    {
        ...
        if (!is_dir($directory) && !mkdir($directory, 0755, \true) && !is_dir($directory)) {
            throw new FileDownloadException("Failed to create directory '{$directory}'.");
        }
    }
    /**
     * @return string
    ...
    private function generateTemporaryFilePath(): string
    {
        ...
        throw new FileDownloadException("Unable to generate a unique temporary file name after " . self::MAX_UNIQUE_ID_ATTEMPTS . " attempts.");
    }
    /**
     * @return string
     */
    ...
    private function createTruncatedFile(int $size): void
    {
        ...
        if ($handle === \false) {
            throw new FileDownloadException("Failed to open temporary file '{$this->temporaryFilePath}' for writing.");
        }
        $this->handle = $handle;
        if (!ftruncate($this->handle, $size)) {
            throw new FileDownloadException("Failed to allocate {$size} bytes for temporary file.");
        }
    }
    /**
     * @return void
    ...
    private function replaceDestinationFile(): void
    {
        if (file_exists($this->destination)) {
            ...
            if (!unlink($this->destination)) {
                throw new FileDownloadException("Failed to delete existing file '{$this->destination}'.");
            }
        }
        if (!rename($this->temporaryFilePath, $this->destination)) {
            throw new FileDownloadException("Unable to rename the file '{$this->temporaryFilePath}' to '{$this->destination}'.");
        }
    }
    /**
     * @param array $context
    ...
    private function cleanupAfterFailure(array $context): void
    {
        ...
        $isDestinationExistsError = str_contains($reason, "The destination '{$this->destination}' already exists.");
