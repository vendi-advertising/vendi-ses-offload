...
final class ResumableDownload extends AbstractResumableTransfer
{
     ...
     */
    public function __construct(string $resumeFilePath, array $requestArgs, array $config, array $currentSnapshot, array $initialRequestResult, array $partsCompleted, int $totalNumberOfParts, ?string $temporaryFile, string $eTag, int $objectSizeInBytes, int $fixedPartSize, string $destination)
    {
    ...
    public static function fromJson(string $json): self
    {
        ...
        if (!is_array($data)) {
            throw new S3TransferException('Invalid resume file format: expected JSON object');
        }
        // Validate version
        if (!isset($data['version']) || $data['version'] !== self::VERSION) {
            throw new S3TransferException('Invalid or unsupported resume file version');
        }
        // Validate required fields
        $requiredFields = ['resumeFilePath', 'requestArgs', 'config', 'initialRequestResult', 'currentSnapshot', 'partsCompleted', 'totalNumberOfParts', 'temporaryFile', 'eTag', 'objectSizeInBytes', 'fixedPartSize', 'destination'];
        foreach ($requiredFields as $field) {
            if (!array_key_exists($field, $data)) {
                throw new S3TransferException("Invalid resume file: missing required field '{$field}'");
            }
        }
        return new self($data['resumeFilePath'], $data['requestArgs'], $data['config'], $data['currentSnapshot'], $data['initialRequestResult'], $data['partsCompleted'], $data['totalNumberOfParts'], $data['temporaryFile'], $data['eTag'], $data['objectSizeInBytes'], $data['fixedPartSize'], $data['destination']);
    }
    /**
    ...
    public static function fromFile(string $filePath): self
    {
        if (!file_exists($filePath)) {
            throw new S3TransferException("Resume file does not exist: {$filePath}");
        }
        $content = file_get_contents($filePath);
        if ($content === \false) {
            throw new S3TransferException("Failed to read resume file: {$filePath}");
        }
        $fileData = json_decode($content, \true);
        if (json_last_error() !== \JSON_ERROR_NONE) {
            throw new S3TransferException('Failed to parse resume file: ' . json_last_error_msg());
        }
        // Validate signature if present
        if (isset($fileData['signature'], $fileData['data'])) {
            $expectedSignature = hash(self::SIGNATURE_CHECKSUM_ALGORITHM, json_encode($fileData['data'], \JSON_PRETTY_PRINT | \JSON_UNESCAPED_SLASHES));
            if (!hash_equals($fileData['signature'], $expectedSignature)) {
                throw new S3TransferException('Resume file integrity check failed: signature mismatch');
            }
