...
abstract class AbstractResumableTransfer
{
     ...
     */
    public function __construct(string $resumeFilePath, array $requestArgs, array $config, array $currentSnapshot)
    {
     ...
     */
    abstract public static function fromFile(string $filePath): self;
    /**
    ...
    public function toFile(?string $filePath = null): void
    {
        ...
        if (!is_dir($resumeDir) && !mkdir($resumeDir, 0755, \true)) {
        ...
        $dataWithSignature = json_encode(['signature' => $signature, 'data' => json_decode($json, \true)], \JSON_PRETTY_PRINT | \JSON_UNESCAPED_SLASHES);
        $result = file_put_contents($saveFileToPath, $dataWithSignature, \LOCK_EX);
        if ($result === \false) {
            throw new S3TransferException("Failed to write resume file: {$saveFileToPath}");
        }
    }
    /**
