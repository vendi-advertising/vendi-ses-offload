...
final class ResumableUpload extends AbstractResumableTransfer
{
     ...
     */
    public function __construct(string $resumeFilePath, array $requestArgs, array $config, array $currentSnapshot, string $uploadId, array $partsCompleted, string $source, int $objectSize, int $partSize, bool $isFullObjectChecksum)
    {
    ...
    public static function fromJson(string $json): self
    {
        ...
        $requiredFields = ['version', 'resumeFilePath', 'requestArgs', 'config', 'currentSnapshot', 'uploadId', 'partsCompleted', 'source', 'objectSize', 'partSize', 'isFullObjectChecksum'];
        foreach ($requiredFields as $field) {
            if (!array_key_exists($field, $data)) {
                throw new S3TransferException("Invalid resume file: missing required field '{$field}'");
            }
        }
        return new self($data['resumeFilePath'], $data['requestArgs'], $data['config'], $data['currentSnapshot'], $data['uploadId'], $data['partsCompleted'], $data['source'], $data['objectSize'], $data['partSize'], $data['isFullObjectChecksum']);
    }
    /**
