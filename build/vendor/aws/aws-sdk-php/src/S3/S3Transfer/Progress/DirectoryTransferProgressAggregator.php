...
final class DirectoryTransferProgressAggregator extends AbstractTransferListener
{
    ...
    public function __construct(string $identifier, int $totalBytes, int $totalFiles, array $directoryListeners = [], ?AbstractTransferListener $directoryProgressTracker = null)
    {
    ...
    {
        $this->directoryNotifier->transferInitiated([self::REQUEST_ARGS_KEY => $requestArgs, self::PROGRESS_SNAPSHOT_KEY => $this->getSnapshot()]);
    }
    /**
    ...
    public function notifyDirectoryComplete(?array $response = null): void
    {
        ...
        $this->directoryNotifier->transferComplete([self::REQUEST_ARGS_KEY => [], self::PROGRESS_SNAPSHOT_KEY => $snapshot]);
    }
    /**
    ...
    {
        $snapshot = $this->getSnapshot();
        $this->directoryNotifier->transferFail([self::REQUEST_ARGS_KEY => [], self::PROGRESS_SNAPSHOT_KEY => $snapshot, self::REASON_KEY => $reason]);
    }
    /**
    ...
    public function bytesTransferred(array $context): bool
    {
        ...
        $this->directoryNotifier->bytesTransferred([self::REQUEST_ARGS_KEY => $context[self::REQUEST_ARGS_KEY] ?? [], self::PROGRESS_SNAPSHOT_KEY => $this->getSnapshot()]);
        return \true;
    }
    /**
     * @inheritDoc
    ...
    public function transferFail(array $context): void
    {
        ...
        $snapshot = $context[self::PROGRESS_SNAPSHOT_KEY];
        $this->markObjectTerminal($snapshot);
        $this->directoryNotifier->bytesTransferred([self::REQUEST_ARGS_KEY => $context[self::REQUEST_ARGS_KEY] ?? [], self::PROGRESS_SNAPSHOT_KEY => $this->getSnapshot(), self::REASON_KEY => $context[self::REASON_KEY] ?? null]);
    }
    /**
    ...
    {
        return new DirectoryTransferProgressSnapshot($this->identifier, $this->transferredBytes, $this->totalBytes, $this->transferredFiles, $this->totalFiles);
    }
    /**
    ...
    private function markObjectTerminal(TransferProgressSnapshot $snapshot): void
    {
        ...
        if (!($this->objectTerminal[$identifier] ?? \false)) {
            $this->objectTerminal[$identifier] = \true;
