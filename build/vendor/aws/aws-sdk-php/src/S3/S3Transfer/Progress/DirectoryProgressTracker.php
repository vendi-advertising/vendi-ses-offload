...
use Vendi\SesOffload\Vendor\Aws\S3\S3Transfer\Exception\ProgressTrackerException;
use Vendi\SesOffload\Vendor\Aws\S3\S3Transfer\Progress\ConsoleProgressBar;
use Vendi\SesOffload\Vendor\Aws\S3\S3Transfer\Progress\DirectoryTransferProgressSnapshot;
use Vendi\SesOffload\Vendor\Aws\S3\S3Transfer\Progress\ProgressBarInterface;
/**
...
final class DirectoryProgressTracker extends AbstractTransferListener implements ProgressTrackerInterface
{
    ...
    public function __construct(ProgressBarInterface $progressBar = new ConsoleProgressBar(), mixed $output = \STDOUT, bool $clear = \true, ?DirectoryTransferProgressSnapshot $currentSnapshot = null, bool $showProgressOnUpdate = \true)
    {
    ...
    private function updateProgressBar(bool $forceCompletion = \false): void
    {
        ...
        $this->progressBar->getProgressBarFormat()->setArgs(['transferred' => min($this->currentSnapshot->getTransferredBytes(), $this->currentSnapshot->getTotalBytes()), 'to_be_transferred' => $this->currentSnapshot->getTotalBytes(), 'unit' => 'B']);
