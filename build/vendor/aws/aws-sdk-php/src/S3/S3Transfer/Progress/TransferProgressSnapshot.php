...
final class TransferProgressSnapshot
{
    ...
    {
        return ['identifier' => $this->identifier, 'transferredBytes' => $this->transferredBytes, 'totalBytes' => $this->totalBytes, 'reason' => $this->reason, 'response' => $this->response];
    }
    /**
    ...
    {
        return new self($this->identifier, $this->transferredBytes, $this->totalBytes, $response);
    }
    /**
     * @param array $data
     *
    ...
    {
        return new self($data['identifier'] ?? null, $data['transferredBytes'] ?? 0, $data['totalBytes'] ?? 0, $data['response'] ?? null, $data['reason'] ?? null);
    }
}
