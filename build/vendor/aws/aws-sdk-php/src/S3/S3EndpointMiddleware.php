...
use Vendi\SesOffload\Vendor\GuzzleHttp\Exception\InvalidArgumentException;
...
class S3EndpointMiddleware
{
    ...
    private function applyPathStyleEndpointCustomizations(CommandInterface $command, RequestInterface $request)
    {
        if ($command->getName() == 'WriteGetObjectResponse') {
            ...
            $fips = \Vendi\SesOffload\Vendor\Aws\is_fips_pseudo_region($this->region) ? "-fips" : "";
            $region = \Vendi\SesOffload\Vendor\Aws\strip_fips_pseudo_regions($this->region);
