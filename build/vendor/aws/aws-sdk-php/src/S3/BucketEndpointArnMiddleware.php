...
use Vendi\SesOffload\Vendor\Aws\Arn\AccessPointArnInterface;
use Vendi\SesOffload\Vendor\Aws\Arn\ArnParser;
use Vendi\SesOffload\Vendor\Aws\Arn\ObjectLambdaAccessPointArn;
use Vendi\SesOffload\Vendor\Aws\Arn\Exception\InvalidArnException;
use Vendi\SesOffload\Vendor\Aws\Arn\AccessPointArn as BaseAccessPointArn;
use Vendi\SesOffload\Vendor\Aws\Arn\S3\OutpostsAccessPointArn;
use Vendi\SesOffload\Vendor\Aws\Arn\S3\MultiRegionAccessPointArn;
use Vendi\SesOffload\Vendor\Aws\Arn\S3\OutpostsArnInterface;
...
use Vendi\SesOffload\Vendor\Aws\Exception\UnresolvedEndpointException;
use Vendi\SesOffload\Vendor\Aws\S3\Exception\S3Exception;
...
class BucketEndpointArnMiddleware
{
    ...
    private function generateAccessPointHost(BaseAccessPointArn $arn, RequestInterface $req)
    {
        ...
        $region = \Vendi\SesOffload\Vendor\Aws\strip_fips_pseudo_regions($region);
    ...
    private function validateArn($arn)
    {
        if ($arn instanceof AccessPointArnInterface) {
            ...
            if (!$clientPart->isRegionMatch($this->region, 's3')) {
                $clientPart = $this->partitionProvider->getPartition(\Vendi\SesOffload\Vendor\Aws\strip_fips_pseudo_regions($this->region), 's3');
            }
            if (!$arn instanceof MultiRegionAccessPointArn) {
