...
use Vendi\SesOffload\Vendor\Aws\Api\Shape;
use Vendi\SesOffload\Vendor\Aws\Api\StructureShape;
use Vendi\SesOffload\Vendor\Aws\CommandInterface;
use Vendi\SesOffload\Vendor\Aws\EndpointV2\EndpointV2SerializerTrait;
use Vendi\SesOffload\Vendor\Aws\EndpointV2\Ruleset\RulesetEndpoint;
use DateTimeInterface;
use Vendi\SesOffload\Vendor\GuzzleHttp\Psr7;
use Vendi\SesOffload\Vendor\GuzzleHttp\Psr7\Request;
use Vendi\SesOffload\Vendor\GuzzleHttp\Psr7\Uri;
...
abstract class AbstractRpcV2Serializer
{
    ...
    public function __invoke(CommandInterface $command, mixed $endpoint = null)
    {
        ...
        $requestTarget = $this->buildRequestTarget($commandName, $operation['http']['requestUri'] ?? '');
     ...
     * @return string
     */
    abstract public function serialize(StructureShape $inputShape, array $commandArgs): string;
    /**
     ...
     * @return array
     */
    abstract protected function resolveTimestamp(int|float|string|DateTimeInterface $value): array;
    /**
            ...
            case 'structure':
                ...
                foreach ($value as $k => $v) {
                    if ($v !== null && $shape->hasMember($k)) {
                        ...
                        $data[$valueShape['locationName'] ?: $k] = $this->resolveInputShape($valueShape, $v);
                    }
                }
                return $data;
            case 'list':
     ...
     * @return string
     */
    private function buildRequestTarget(string $commandName, string $requestUri): string
    {
        $requestUri = str_ends_with($requestUri, '/') ? $requestUri : $requestUri . '/';
