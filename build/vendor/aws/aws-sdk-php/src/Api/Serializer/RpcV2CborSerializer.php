...
namespace Vendi\SesOffload\Vendor\Aws\Api\Serializer;

use Vendi\SesOffload\Vendor\Aws\Api\Cbor\CborEncoder;
use Vendi\SesOffload\Vendor\Aws\Api\Cbor\Exception\CborException;
use Vendi\SesOffload\Vendor\Aws\Api\Exception\RpcV2CborException;
...
{
    /** @var array|string[]  */
    protected static array $defaultHeaders = [self::HEADER_SMITHY_PROTOCOL => 'rpc-v2-cbor', self::HEADER_CONTENT_TYPE => 'application/cbor', self::HEADER_ACCEPT => 'application/cbor'];
    /** @var CborEncoder  */
     ...
     */
    public function serialize(StructureShape $inputShape, array $commandArgs): string
    {
        try {
            $resolvedInput = $this->resolveInputShape($inputShape, $commandArgs);
            return !empty($resolvedInput) ? $this->encoder->encode($resolvedInput) : $this->encoder->encodeEmptyIndefiniteMap();
        } catch (CborException $e) {
            throw new RpcV2CborException('Unable to encode CBOR document ' . $inputShape->getName() . ': ' . $e->getMessage() . \PHP_EOL);
        }
    }
    /**
    ...
    protected function resolveBlob(mixed $value): array
    {
        if (is_resource($value)) {
            ...
            if ($value === \false) {
                throw new RpcV2CborException('Failed to read resource stream value during serialization');
            }
        }
     ...
     * @throws RpcV2CborException
     */
    protected function resolveTimestamp(int|float|string|DateTimeInterface $value): array
    {
        if (is_numeric($value)) {
        ...
        if ($value instanceof DateTimeInterface) {
            ...
            $value = $value->getTimestamp() + $micro / 1000000.0;
        } else {
            $timestamp = strtotime($value);
            if ($timestamp === \false) {
                throw new RpcV2CborException('Request serialization failed: Invalid date/time: ' . $value);
            }
            $value = $timestamp;
        }
