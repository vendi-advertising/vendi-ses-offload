...
trait RpcV2ParserTrait
{
        ...
        switch ($shape['type']) {
            ...
            case 'timestamp':
                try {
                    ...
                    trigger_error('Unable to parse timestamp value for ' . $shape->getName() . ': ' . $e->getMessage(), \E_USER_WARNING);
                }
                return $value;
            default:
                return $value;
     ...
     * @return mixed
     */
    protected function parseCbor(StreamInterface $stream, ResponseInterface $response): mixed
    {
        try {
            $cborString = (string) $stream;
            return empty($cborString) ? null : $this->decoder->decode($cborString);
        } catch (CborException $e) {
            throw new ParserException("Malformed Response: error parsing CBOR: {$e->getMessage()}", 0, $e, ['response' => $response]);
        }
    }
}
