...
abstract class AbstractRpcV2Parser extends AbstractParser
{
     ...
     * @return Result
     */
    private function parseResponse(ResponseInterface $response, Operation $operation): Result
    {
        ...
        if ($smithyProtocolHeader !== static::$smithyProtocol) {
            ...
            throw new ParserException("Malformed response: Smithy-Protocol header mismatch (HTTP {$statusCode}). " . 'Expected ' . static::$smithyProtocol);
        }
        ...
        $result = $this->parseMemberFromStream($response->getBody(), $outputShape, $response);
