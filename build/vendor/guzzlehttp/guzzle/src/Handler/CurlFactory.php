...
namespace Vendi\SesOffload\Vendor\GuzzleHttp\Handler;
...
use Vendi\SesOffload\Vendor\GuzzleHttp\Promise as P;
use Vendi\SesOffload\Vendor\GuzzleHttp\Promise\FulfilledPromise;
...
use Vendi\SesOffload\Vendor\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \Vendi\SesOffload\Vendor\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \Vendi\SesOffload\Vendor\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \Vendi\SesOffload\Vendor\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \Vendi\SesOffload\Vendor\GuzzleHttp\Psr7\Utils::streamFor($sink);
